...
public class Calculator_Gui extends JFrame implements ActionListener, KeyListener{
	...
	private GradualCredit gCredit = new GradualCredit(); // 졸업 기준 학점 저장(서버넘김용)-hyseo
	private GradualStatus gStatus = new GradualStatus(); // 졸업 관련 정보 저장(서버 > ui전달용)-hyseo
	private GraduationCal gCal = new GraduationCal(); // 졸업 정보 계산기 - hyseo
	...
	public void actionPerformed(ActionEvent e) { 
		try {
			if (e.getSource() == enter) {
	            ...
	            // 졸업 학점 저장 (서버 넘겨줄 용도) - hyseo
	            gCredit.setTotalCredit(graduationCredit);
	            ...
	            // 졸업 상태 계산 - hyseo
                gCal.setGraduationStatus(subjectlist, gCredit, gStatus);
	...
	private void showResultWindow(String name, double avg, int retakeCount, double totalCredit, double graduationCredit) {
	    ...
	    gradBtn.addActionListener(new ActionListener() {
	    	public void actionPerformed(ActionEvent e) {
	    		new graduationCreditGui(name, avg, (int)retakeCount, subjectlist, gStatus);
	    	}
	    });
